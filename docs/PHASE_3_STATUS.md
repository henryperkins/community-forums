...
- **Automated suite:** `composer test` → **392 tests / 1347 assertions green** (PHP 8.5, MySQL). Baseline before Phase 3 was 275/987. (Green ≠ at-bar: the suite covers what was built, not the unbuilt Gate A sub-requirements catalogued in §11.)
...
| P3-01 | Preferences & settings IA | **Partial** | Done: versioned schema/defaults/reset/**export**/**upgrade path**, appearance enforcement, reading pagination, reading display toggles + thread sort server-enforced, **malformed-JSON recovery tested**. **Only gap (§11):** composing toggles (`enter_to_send`/`show_preview`/`smart_lists`) still write-only — need composer JS. `PreferenceSchemaTest.php`, `AppUserPreferencesTest.php`, `AppReadingPreferencesTest.php` |
...
| P3-05 | Rate limits & anti-abuse + IP purge | **Partial** (engine done; controls/seam open) | Done & tested: RateLimitService, AntiAbuseService (observe→flag→hold→block), approval holds, proxy-aware ClientIdentifier, audited system actions, IP-purge worker. **Gaps (§11):** spam-scoring **seam absent** (a Gate A item); **no admin UI** for registration mode (dead/unenforced setting), anti-abuse mode/blocked-words, or board `require_approval`; central limiter **bypassed** by login/DM/password-reset (raw REMOTE_ADDR → collapses behind a proxy); first-post hold missing. `RateLimitServiceTest.php`, `AppContentApprovalTest.php`, `AppAutomationAuditTest.php`, `IpRetentionPurgeTest.php`; `/mod/approvals`; `worker:purge-ips` |
...
|---|---|---|
| P3-01 | Partial | **Only gap left:** composing (`enter_to_send`/`show_preview`/`smart_lists`) toggles still write-only — need composer JS (not server-only). **CLOSED 2026-06-27:** reading toggles + thread sort server-enforced (`AppReadingPreferencesTest`); preference **export** (`AppUserPreferencesTest`); **schema upgrade path** (`PreferenceSchema::upgrade` — version-stepped + legacy-value normalization, wired into `resolve()`) and **malformed-JSON recovery**, both tested (`PreferenceSchemaTest`, `AppUserPreferencesTest`) |
...
| P3-05 | Partial | **Spam-scoring provider seam** absent (a Gate A commitment, not Gate B); **no admin UI** for registration mode (dead/unenforced setting), anti-abuse mode + blocked-words, or board `require_approval`; central limiter **bypassed** by login/DM/password-reset (ad-hoc constants on raw REMOTE_ADDR → collapses behind a proxy); optional first-post hold missing |
