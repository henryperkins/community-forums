...
| D4 | Page head | constraint | One h1 `Email & announcements` for both tabs (design:35) | Two real routes, two h1s: `Email delivery` (`email.php:14`), `Announcements` (`announcements.php:10`) | Keep per-page h1s (DESIGN §5.3: every view has a real shareable URL). | low |
...
**Tests:** none — the ADR *is* the evidence, per DESIGN §13 deferral discipline ("never silently dropped").
