...
- **Plain-text paste** → read as **Markdown**, because Markdown *is* the canonical stored format: pasting a Markdown document into rich mode yields headings/lists/quotes/marks, not literal `##` and `**`. Without this the serializer backslash-escapes every construct the author pasted and the published post renders as literal syntax. Two escapes stay literal: a paste landing inside a fence or an inline-code run, and `Cmd/Ctrl+Shift+V` (§12).
- **Image paste** (clipboard) → uploaded and inserted as an image (§7).
- **URL paste over a selection** → wraps the selection in a link.
- **Plain URL paste** → auto-links; optional unfurl/embed at render time (§7). A same-origin board/tag/topic URL is normalized to its canonical reference chip and keeps that path ahead of Markdown parsing.
...
| v0.10 | 2026-08-09 | §7 link-unfurl row moved from "planned P2" to shipped: server-side unfurl behind three gates (flag, per-board opt-in, SSRF host allowlist), public boards only, never DMs, with author removal that survives an edit. ADR 0025; runbook `docs/runbooks/link_previews.md`. |
| v0.9 | 2026-08-03 | §3.3 gains the **plain-text paste = Markdown** rule. Rich mode previously let ProseMirror take a pasted Markdown document as literal text, one paragraph per line; the serializer then backslash-escaped every construct, so `posts.body` stored `\## Heading` and the published post rendered literal syntax (observed in production). Plain-text pastes now parse through the editor's own CommonMark parser, with fences/inline-code and `Cmd/Ctrl+Shift+V` as the literal escapes and internal-URL chip rewriting keeping priority. Covered by `wysiwyg-composer.spec.ts` (paste → rich structure → submitted post, plus both escapes). |
