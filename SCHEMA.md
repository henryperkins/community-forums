...
- `identity_providers(id, provider_key, display_name, protocol, type, issuer, discovery_url, jwks_uri, jwks_cache_json, jwks_cached_at, discovery_cache_json, discovery_cached_at, client_id, client_secret_ref, claim_map_json, is_enabled, health_status, health_checked_at, created_at, updated_at)` with unique `provider_key`; **seeds google/apple/github** as dark `builtin` rows. `client_secret_ref` is a reference into the encrypted secret service — **never plaintext**. `discovery_cache_json`/`discovery_cached_at` (added `0074`) cache the OIDC discovery document beside the 0052 JWKS cache. Animated by Increment 8 (2026-07-07): generic-OIDC rows (`type='generic_oidc'`) power configuration-only providers behind the dark `provider_registry` flag; `0074` also backfills `oauth_identities.provider_config_id` from `provider_aliases` (idempotent DML; uniqueness/resolution still derive from the provider string — keys are 1:1 with registry rows, satisfying the decision-#32 `(provider_config, sub)` identity key without repointing the physical unique key).
...
- `invitations(id, token_hash, created_by, email, domain, onboarding_role_id, onboarding_board_id, max_uses, used_count, expires_at, revoked_at, revoked_by, created_at)` with unique `token_hash` (sha256 — **no raw token column**) and creator/revoker/role/board FKs; an invitation is onboarding evidence, not authority (`onboarding_role_id` is non-privileged only, enforced in code). Animated by Increment 9 (2026-07-08) behind the dark `invitations` flag: admin console issuance/revoke + invite-mode registration; `onboarding_role_id` is stored but **never applied** by Inc 9 redemption (no approved onboarding-role policy — `docs/phase5/invitation-defaults.md`), while `onboarding_board_id` grants board membership atomically with account creation.
...
| Version | Date | Notes |
|---|---|---|
| v1.36 | 2026-07-08 | Phase 5 Increment 9 migration `0076_phase5_invitation_audit`: widened `moderation_log.target_type` with `invitation` (issuance/revoke/redemption audit rows — mirrors the 0075 `identity_provider` widen). `invitations`/`invitation_redemptions` rows are now animated by the P5-13 console + invite-mode registration behind the dark `invitations` flag; `registration_mode` gains the explicit `invite` value (settings-backed, no schema change). |
