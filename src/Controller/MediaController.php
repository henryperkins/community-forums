...
final class MediaController extends Controller
{
    ...
    public function show(Request $request, array $params): Response
    {
        ...
        $private = $att['visibility'] === 'private' || $att['status'] === 'temp';
        return (new Response($bytes, 200, [
            'Content-Type' => (string) $att['mime'],
            'Content-Length' => (string) strlen($bytes),
            'Content-Disposition' => 'inline',
            // Private media is never shared-cacheable; public media may be cached.
            'Cache-Control' => $private ? 'private, no-store' : 'public, max-age=31536000, immutable',
        ]));
    }

    /** @param array<string,mixed> $att */
    private function authorize(array $att): void
    {
        $user = $this->currentUser();

        // The owner can always see their own upload (incl. temp/pending).
        if ($user !== null && (int) $att['user_id'] === $user->id()) {
            return;
        }

        // Unbound temp uploads are visible only to their owner.
        if ($att['status'] === 'temp') {
            throw new NotFoundException('Media not found.');
        }

        // DM media: only conversation participants.
