...
> additive and now default-on after Gate A/B2 acceptance. They are consumed only
> through `SecretVault` while the `service_secrets` flag controls write/rotation
> availability and rollback. Webhook endpoint config consumes it; provider and
> remote-app consumers remain deferred.
>
> Table 83 and the reconciled `webhooks` shape are the **B2 webhook delivery
> engine** (migration `0057`): default-on after Gate A/B2 acceptance via
> `webhooks`, with endpoint signing secrets stored as SecretVault references and
> a durable retry/dead-letter ledger.
>
> The **B2 first-party hook registry** adds no schema: it is a code-only,
> default-on producer/listener seam via `first_party_hooks`. Its first listener
> maps public-board domain events into the existing webhook delivery ledger;
> `ping` remains an admin test event.
>
...
CREATE TABLE moderation_log (
  ...
  target_type ENUM('thread','post','user','board','category','setting','service_secret','api_token','webhook','registry','package','publisher','identity_provider','invitation') NOT NULL,  -- widened by 0057/0068/0070 (…registry/package), 0073 (+publisher), 0075 (+identity_provider), 0076 (+invitation)
  target_id   BIGINT UNSIGNED NOT NULL,
...
- `link_previews(id, source_type, source_id, url, url_hash, final_url, status, title, description, image_url, site_name, http_status, metadata, error, removed_by, removed_at, fetched_at, purged_at, created_at, updated_at)` with unique `(source_type, source_id, url_hash)` and the `(status, created_at)` queue index. The `(source_type, source_id)` source-lookup index `0058` also created is a prefix of that unique key and is dropped by migration `0080`; source lookups resolve on the unique key. `status` is `ENUM('queued','fetched','blocked','failed','purged','removed')` — migration `0081` added the `removed` member plus `removed_by`/`removed_at` for author suppression, which is sticky across the queue upsert (`purged` is revived by it, `removed` is not).
...
- `user_profile_fields(id, user_id, label, value, position, created_at, updated_at)` (`0062`) with unique `(user_id, position)` and user FK (CASCADE) — bounded (≤3) extra public profile fields; `label VARCHAR(40)`, `value VARCHAR(160)`. `0062` also created a duplicate `KEY idx_user_profile_field_user (user_id, position)`; migration `0079` drops it — the unique key has the same tuple, so it already serves the `user_id`-ordered read and covers the FK.
...
undocumented table/column. It was originally inert until each increment wired
behavior behind its flag; accepted Gate A workstreams now default on for fresh
installs as of 2026-07-09 and remain reversible through `features` overrides.
Gate B (`server_extensions`, `governance`, `service_principals`,
`verified_links`) remains reserved and default-off. Per `PHASE_5_PLAN` §2/§7,
the registry trust roots, signing-key custody, WebAuthn RP ID, OIDC provider
...
- `identity_providers(id, provider_key, display_name, protocol, type, issuer, discovery_url, jwks_uri, jwks_cache_json, jwks_cached_at, discovery_cache_json, discovery_cached_at, client_id, client_secret_ref, claim_map_json, is_enabled, health_status, health_checked_at, created_at, updated_at)` with unique `provider_key`; **seeds google/apple/github** as builtin rows controlled by the legacy `oauth` flag/config. `client_secret_ref` is a reference into the encrypted secret service — **never plaintext**. `discovery_cache_json`/`discovery_cached_at` (added `0074`) cache the OIDC discovery document beside the 0052 JWKS cache. Animated by Increment 8 (2026-07-07): generic-OIDC rows (`type='generic_oidc'`) power configuration-only providers through the accepted `provider_registry` flag; `0074` also backfills `oauth_identities.provider_config_id` from `provider_aliases` (idempotent DML; uniqueness/resolution still derive from the provider string — keys are 1:1 with registry rows, satisfying the decision-#32 `(provider_config, sub)` identity key without repointing the physical unique key).
...
- `invitations(id, token_hash, created_by, email, domain, onboarding_role_id, onboarding_board_id, max_uses, used_count, expires_at, revoked_at, revoked_by, created_at)` with unique `token_hash` (sha256 — **no raw token column**) and creator/revoker/role/board FKs; an invitation is onboarding evidence, not authority (`onboarding_role_id` is non-privileged only, enforced in code). Animated by Increment 9 (2026-07-08) through the accepted `invitations` flag: admin console issuance/revoke + invite-mode registration; `onboarding_role_id` is stored but **never applied** by Inc 9 redemption (no approved onboarding-role policy — `docs/phase5/invitation-defaults.md`), while `onboarding_board_id` grants board membership atomically with account creation.
...
The following candidates remain unspecced/uncommitted as of 2026-09-23. A mention in a surface spec is not approval to add schema.
...
**Post-submission idempotency is no longer a candidate.** Migration `0044_submission_idempotency.php` added the durable idempotency ledger used by posts, DMs, edits, and other submissions; its current table shape is defined above. `email_deliveries.idempotency_key` remains a separate per-delivery deduplication mechanism.
...
| v1.39 | 2026-08-03 | Privacy-remediation migration `0078_purge_since_last_read_context` deletes the regenerable `since_last_read_context` cache so rows created before canonical anonymous-author masking cannot retain usernames or display names. No table shape changes. Rollback is intentionally a no-op because purged cache data is non-authoritative and cannot be reconstructed safely. |
...
| v1.15 | 2026-06-28 | **Phase 5 foundation (Milestone 1 schema reconciliation).** Documented additive migrations `0049`–`0053` in new **§5A**: registry/packages/releases/installs/permissions/history/advisories/local-blocks (§8.2 #1–5), capability registry + protected roles (4 seeded system anchors) + scoped assignments/audit + protected-owner authority (§8.2 #8/#9/#13), WebAuthn credentials/challenges (§8.2 #14), identity-provider registry + the `oauth_identities.provider` **ENUM→VARCHAR(64)** widen + `provider_config_id` linkage (§8.2 #15), and invitations/redemptions (§8.2 #16). Added table-index rows 55–77. Tables were originally inert (no app reads/writes before their increments) and carried **no** Milestone-0 trust policy. |
...
| v1.8 | 2026-06-26 | Added a §6 pointer to the new **docs/history/PHASE_1_MIGRATIONS.md** (the authoritative Phase‑1 column cut + migration order). No schema change. |
