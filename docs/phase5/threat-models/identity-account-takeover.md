...
| TM-ID-09 | Last usable credential removal strands account. | Last-usable-method removal blocked; the Inc8 handoff (provider-disable lists sole-method accounts first) is resolved — `/admin/providers/{id}/disable` confirm page, `AppAdminProvidersTest` + `providers.spec.ts`. | Inc7 |
