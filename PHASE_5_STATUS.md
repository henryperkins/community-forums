...
evidence has now been collected for GA-DOD-17 through GA-DOD-22. Gate A still
requires explicit product-owner acceptance (GA-DOD-23) before it can be called
accepted; staged enablement (R5) remains per subsystem. Gate B workstreams
(P5-05/06 sandbox, P5-10 governance, P5-14/15) stay reserved behind their dark
flags.
...
Gate A is still not product-owner accepted until `docs/adr/0017-phase-5-gate-a-closeout.md` records explicit acceptance. Gate B workstreams (`server_extensions`, `governance`, `service_principals`, `verified_links`) remain reserved and are not silently completed by this closeout.
