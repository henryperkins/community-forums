...
- **Inbox prerequisite (guarded):** `composer.js` exports `RetroBoardsComposer.enhanceWithin(root)` (idempotent via the existing `data-rb-enhanced` guard). The Inbox `loadThread` success path in `app.js` calls it **behind a namespace guard — `window.RetroBoardsComposer && window.RetroBoardsComposer.enhanceWithin(root)` — because `app.js` loads unconditionally (`layout.php:79`) while `composer.js` loads only when `rich_composer` is on (`layout.php:80`);** an unguarded call under the kill switch would throw inside the `.then` chain and trip the `.catch` full-page fallback (`app.js:576`), defeating in-pane loading in exactly the configuration the kill switch protects. This matches the existing namespace-guard precedent (`wysiwyg-composer.js` checking `window.RetroBoardsComposer` before registering).
...
- All writes remain CSRF-protected form POSTs; no GET mutates state; the idempotency key becomes server-rendered in **all** mounts.
...
- attach: `＋` uploads via file picker (accept list per DECISIONS §6 #6); chip row renders inside the box; progress styled and announced;
...
- `public/assets/composer.js` (icon toolbar + overflow, action-bar injection, context-aware Enter + in-flight guard, `:` trigger, emoji dialog, attach, popover positioning + retargeted insertion points per §6, counter, draft row, guarded-`enhanceWithin` export), `public/assets/app.js` (guarded Inbox call site), `public/assets/app.css`;
...
No migration, route, or feature-flag change is expected. Service and adapter changes are limited to the additive extensions named above (the suggestion-service `:` arm and the `enterShouldSubmit()` adapter hook, which extends the duck-typed adapter contract documented in tech-debt #8's notes).
