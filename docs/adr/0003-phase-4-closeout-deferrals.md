...
| Automated since-last-read context and scheduled related-topic refresh | Product + Engineering | Phase 4 follow-on graduation via ADR 0019 | **Resolved 2026-07-12:** since-last-read context uses local read/post state, `worker:related-topics` creates deterministic tag-related public-thread links, and the bounded Thread Intelligence evidence package passed. | `automated_context` graduated default-on with `community_memory`; retain independent explicit-false rollback and the ADR 0019 public-only processor boundary. |
