...
> **Status: Phase 5 (ecosystem, identity & governance) — Gate A build in progress; everything deploy-dark.** Phase 4 closed with explicit deferrals ([`docs/adr/0003-phase-4-closeout-deferrals.md`](docs/adr/0003-phase-4-closeout-deferrals.md)), and most of its deferred surfaces have since graduated default-ON. Phase 5 has landed — all behind default-off feature flags, so live behavior is unchanged until an operator enables them: the signed package-registry protocol, install/update lifecycle, declarative theme packages, the integration runtime + security-response console, the database-backed capability resolver with its enforcement cutover (`CAPABILITIES_MODE`), opt-in TOTP/recovery, WebAuthn passkeys, generic-OIDC identity providers with an operator console, encrypted service secrets, read-only API tokens, and outbound webhook delivery. Suite green at **1760 tests / 9022 assertions** (2026-07-07); resolver parity **1551/1551, 0 mismatches**; populated-upgrade rehearsal **17/17** through migration `0075`. See [`PHASE_5_STATUS.md`](PHASE_5_STATUS.md) and [`docs/phase5/requirement-ledger.json`](docs/phase5/requirement-ledger.json); the Phase 4 record remains in [Phase 4 history](docs/history/PHASE_1-4_HISTORY.md#phase-4-status) and [`docs/evidence/phase4-gate-a.md`](docs/evidence/phase4-gate-a.md).
