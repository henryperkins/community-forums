...
?>
<li class="inbox-thread-row<?= $unread ? ' is-unread' : '' ?><?= $status !== 'open' ? ' thread-status-' . $e($statusClass) : '' ?><?= !empty($t['is_pinned']) ? ' is-pinned' : '' ?><?= !empty($t['is_locked']) ? ' is-locked' : '' ?>" data-inbox-row data-thread-id="<?= $threadId ?>">
    ...
    <?php if ($current_user !== null && $current_user->isActive()): ?>
        <form class="inbox-row-star" method="post" action="/t/<?= $threadId ?>/star">
            <?= $this->csrfField() ?>
    ...
    <details class="inbox-row-menu">
        ...
        <div class="inbox-row-menu-panel">
            <form method="post" action="/t/<?= $threadId ?>/read">
                <?= $this->csrfField() ?>
                <input type="hidden" name="return" value="<?= $e($return_to) ?>">
