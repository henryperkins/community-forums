...
- **Member surfaces** — one per route, in the order you meet them: `board-index` (`/`: the non-personalized place directory, with Tags / Notices / Connections panes) · `forum-inbox` (`/inbox`: the personal queue with independent scope/order and a bounded reading preview) · `search` (`/search`: scoped, ordered results) · `compose` (`/compose`: the top-level new-topic task) · `board-page` (`/c/{slug}`: board masthead over a compact ruled topic list) · `thread-view` (the council topic: post stream, poll, living-brief slot, composer, warden's tools) · `living-brief` (the evidence-bound brief in its three provenance postures) · `user-profile` (gilt cover, regard, marks of esteem, activity tabs) · `users-online` (the presence roll in the shared shell — ForumNav, BoardRail, PresenceList) · `account-settings` (grouped rail, grouped form panels, live two-factor, sessions, connections, guarded delete). The reviewed four-surface source map, comparison screenshots, and non-runtime rule live in `templates/member-surfaces/README.md`.
