...
Delivery was planned as seven **phases**, each a "release train" split into **Gate A** (minimum release) and **Gate B** (extended slice) with entry/exit gates and a carryover ledger. Phases 1–4 are complete and Phase 5 Gate A is accepted and default-on; Phase 5 Gate B and Phases 6–7 have not started. The original plans and status ledgers are **archived, unmaintained history** in `docs/history/` (`PHASE_1_PLAN.md` … `PHASE_7_PLAN.md`, `PHASE_5_STATUS.md`, `PHASE_1-4_HISTORY.md`) — cite them for scope history and gate definitions, never for current state. **There is no single status ledger:** a subsystem's availability is its `FeatureFlags::DEFAULTS` entry plus its runbook in `docs/runbooks/`; decisions, deferrals and open carryovers are ADRs in `docs/adr/` (never silently dropped); what shipped is recorded in `CHANGELOG.md`; proof artifacts live in `docs/evidence/`. Design-decision archives for shipped work live in `docs/superpowers/` (indexed by its `README.md`).
