...
- **Automated suite:** `composer test` → **399 tests / 1375 assertions green** (PHP 8.5, MySQL). Baseline before Phase 3 was 275/987. (Green ≠ at-bar: the suite covers what was built, not the unbuilt Gate A sub-requirements catalogued in §11.)
...
| P3-05 | Rate limits & anti-abuse + IP purge | **Partial** (engine + operator controls done) | Done & tested: RateLimitService, AntiAbuseService (observe→flag→hold→block), approval holds, proxy-aware ClientIdentifier, audited system actions, IP-purge worker, **admin UI for registration mode + anti-abuse mode/blocked-words + board `require_approval`** (registration mode now enforced — was a dead setting). **Gaps (§11):** spam-scoring **seam** absent (a Gate A item); central limiter **bypassed** by login/DM/password-reset (raw REMOTE_ADDR → collapses behind a proxy); first-post hold missing. `RateLimitServiceTest.php`, `AppContentApprovalTest.php`, `AppAutomationAuditTest.php`, `IpRetentionPurgeTest.php`, `AppAdminModerationTest.php`; `/admin` settings; `/mod/approvals`; `worker:purge-ips` |
...
| P3-05 | Partial | **CLOSED 2026-06-27:** admin UI for **registration mode** (open/closed, now enforced in `AuthController` — was a dead setting), **anti-abuse mode + blocked-words**, and the board **`require_approval`** toggle, all audited (`AppAdminModerationTest`). **Remaining:** spam-scoring provider **seam** (a Gate A commitment); central limiter **bypassed** by login/DM/password-reset (raw REMOTE_ADDR → collapses behind a proxy); optional **first-post hold**; approval-mode registration (needs a `users.status='pending'` migration + approval queue) |
