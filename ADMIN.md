...
| **Unfurl link previews** | Per-board opt-in for server-side link previews (DECISIONS §6 #5; `boards.link_previews_enabled`, default off). Public boards only; the host allowlist on the link-previews console still decides what may be fetched. The checkbox renders only while the `link_previews` flag is on, and a board edit made while it is off never revokes a stored opt-in. |
