...
**Phase 4 snapshot (2026-07-12):** accepted baseline on `main`; the checkout reviewed for this record included later deploy-dark carryover code and closeout evidence.
**Suite at that snapshot:** accepted baseline `./vendor/bin/phpunit` → 456 tests / 1635 assertions, green. The checkout's `composer test` → 866 tests / 4519 assertions, green.
...
- `tags`, `expanded_feeds`, and `reputation_ledger` graduated to default-ON on 2026-07-01 (acceptance evidence: `AppFeatureFlagTest`, `AppPhase4GateATest`, `AppFollowFeedTest`, `AppLeaderboardTest`, and `docs/runbooks/phase4-tags-feeds-reputation.md`). Reversible via the `features` override; current design ownership is mapped in `docs/design-system/imladris/github.md`.
- `board_folders`, `bookmark_folders`, and `saved_feeds` graduated to default-ON on 2026-07-01 (acceptance evidence: `AppPhase4CarryoverFoundationTest` and `AppBoardFoldersSavedFeedsTest`). Reversible via the `features` override; current design ownership is mapped in `docs/design-system/imladris/github.md`.
