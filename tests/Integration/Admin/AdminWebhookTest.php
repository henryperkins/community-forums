...
final class AdminWebhookTest extends TestCase
{
    ...
    public function test_register_shows_secret_once_then_hidden(): void
    {
        ...
        self::assertStringContainsString('Copy this signing secret now — it will not be shown again:', $res->body());
    ...
    public function test_toggle_and_send_test_and_delete_round_trip(): void
    {
        ...
        self::assertStringContainsString('Endpoint paused', urldecode(implode(' ', $paused->cookieHeaders())));
    ...
    public function test_secret_store_gate_copy_is_true_of_the_surface_it_lands_on(): void
    {
        // The rotate form attaches the gate to a password input on a page that
        // creates nothing, so it cannot borrow the register form's sentence.
        $this->enable();
        $this->actingAs($this->makeAdmin(['password' => 'password123']));
        $this->register();
        $id = (int) $this->db->fetchValue('SELECT id FROM webhooks WHERE name = ?', ['CI hook']);

        (new SettingRepository($this->db))->set('features', ['webhooks' => true, 'service_secrets' => false]);
        $res = $this->post('/admin/webhooks/' . $id . '/rotate', ['current_password' => 'password123']);

        $this->assertStatus(422, $res);
        self::assertStringContainsString('Enable the service-secret store before rotating a signing secret.', $res->body());
        self::assertStringNotContainsString('before creating webhooks', $res->body());
    }
