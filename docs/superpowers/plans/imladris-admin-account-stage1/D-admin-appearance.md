...
| 4 | h1 | constraint | One h1 `Branding & themes` spanning both tabs (`:35`) | Two h1s: `Branding` (`branding.php:12`), `Themes` (`themes.php:8`) | Keep two h1s. DESIGN §5.3: every view has a real shareable URL; the design's merge is a prototype elision | low |
...
| 68 | dc-runtime behaviour | constraint | `sc-if`/`sc-for`, `onSubmit=`/`onClick=`/`onInput=`/`onChange=`, `ref=`, the whole `x-dc` state machine | Server-rendered PHP + real forms; `public/assets/app.js` decorates only | Every state must be a server render or a real URL. `./support.js` and `./ds-base.js` never ship (DESIGN §6.14) | low |
