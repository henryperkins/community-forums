...
> **Status: Phase 3 (polish, trust & scale) — Gate A core slice implemented on top of Phase 2.** Phase 2 (community essentials) is feature-complete; Phase 3 adds the **appearance/reading/composing settings IA** with a versioned preference schema, the **shared Markdown composer** server pipeline (live preview via the same render+sanitize path, spoilers, `/media` images, idempotent submit), **safe image uploads** (content-sniffed, GD re-encoded, authorization-gated private delivery, orphan sweep), **central anti-abuse** (rate-limit service, word/link/duplicate/flood rules, observe→flag→hold→block modes, an audited approval queue) with a **90-day IP-retention purge** worker, **operator branding** (name/logo/favicon/colors + `/brand.css`), **SEO** (`/sitemap.xml`, `/robots.txt`, canonical/OG/noindex), and a **skippable/replayable onboarding tour**. The automated suite is green at **368 tests / 1248 assertions** (PHP 8.5). Gate B (TOTP, appeals, retro skin/custom CSS, server draft sync, non-image files, plugins/webhooks/API, account polish) remains plan-only. See [`docs/PHASE_3_STATUS.md`](docs/PHASE_3_STATUS.md) for the Phase 3 evidence index + carryover ledger, [`docs/adr/0001-composer-engine.md`](docs/adr/0001-composer-engine.md) for the composer decision, [`docs/PHASE_2_STATUS.md`](docs/PHASE_2_STATUS.md) for the Phase 2 index, and [`docs/PHASE_2_RUNBOOK.md`](docs/PHASE_2_RUNBOOK.md) for operations.
