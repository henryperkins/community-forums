...
| Custom emoji | Graduated default-on 2026-07-03 with operator catalogue, shortcode rendering, optional reactions, rollback tests, browser/a11y evidence, and media moderation runbook | Phase 4 closeout | `custom_emoji` | `AppCustomEmojiGiphyTest`; `AppPhase4CarryoverFoundationTest`; browser `48-custom-emoji-admin`/`49-custom-emoji-thread`; scoped axe scans; `docs/runbooks/custom_emoji.md` | Retained operator rollback via `features.custom_emoji=false` |
...
| Since-last-read context | Deterministic local-data assembly before advancing read state — **graduated default-on with Thread Intelligence 2026-07-12** | Phase 4 closeout | `automated_context` | `AppAutomatedContextTest` zero-override liveness + explicit rollback; browser/no-JS evidence; runbook | None — independently reversible through `features.automated_context=false` |
| Scheduled related-topic refresh | Deterministic tag-based public-thread refresh worker — **graduated default-on with Thread Intelligence 2026-07-12** | Phase 4 closeout | `community_memory`, `automated_context`, `tags` | `RelatedTopicRefreshWorkerTest` zero-override/rollback coverage; populated worker and browser evidence; runbook | None — provider calls still require credentials and both owning flags |
| Thread Intelligence automatic Living Briefs | Complete default-on implementation: public-only evidence builder, leased worker, bounded provider/moderation seams, validated atomic publication, curator lineage/controls, redacted admin operations, UTC budgets, board sweeps, and 90-day unpublished-evidence retention | Phase 4 follow-on graduation | `community_memory`, `automated_context` (both default `true` as of 2026-07-12) | ADR 0019; migration `0077`; `docs/runbooks/thread_intelligence.md`; Task 12 redacted live gate `low`/`16000`, 46/46 runs, 149/149 supported, zero incomplete/private-sentinel/fabricated-decision outcomes; all pre-flip gates indexed | Final post-flip identical-suite evidence marker; runtime remains independently reversible without data deletion |
