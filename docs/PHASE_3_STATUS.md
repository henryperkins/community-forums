...
- **Automated suite:** `composer test` → **385 tests / 1328 assertions green** (PHP 8.5, MySQL). Baseline before Phase 3 was 275/987. (Green ≠ at-bar: the suite covers what was built, not the unbuilt Gate A sub-requirements catalogued in §11.)
...
| P3-01 | Preferences & settings IA | **Partial** | Done: versioned schema/defaults/reset/**export**, appearance enforcement (theme/density/font/motion), reading pagination, **reading display toggles (avatars/signatures/reactions) + thread sort now server-enforced**. **Gaps (§11):** composing toggles still write-only; no upgrade path/test; malformed-JSON recovery untested. `PreferenceSchemaTest.php`, `AppUserPreferencesTest.php`, `AppReadingPreferencesTest.php` |
...
|---|---|---|
| P3-01 | Partial | Composing (`enter_to_send`/`show_preview`/`smart_lists`) toggles still write-only (need composer JS); no upgrade-path migration/test; malformed-JSON recovery untested. **CLOSED 2026-06-27:** reading toggles (`thread_sort`/`show_avatars`/`show_signatures`/`show_reactions`) server-enforced (`AppReadingPreferencesTest`) + preference **export** (`GET /settings/preferences/export`, `AppUserPreferencesTest`) |
