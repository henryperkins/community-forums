...
**Status:** v1.41 · **Owner:** Henry (lakefrontdigital.io) · **Last updated:** 2026-08-04
...
- `link_previews(id, source_type, source_id, url, url_hash, final_url, status, title, description, image_url, site_name, http_status, metadata, error, fetched_at, purged_at, created_at, updated_at)` with unique `(source_type, source_id, url_hash)` and the `(status, created_at)` queue index. The `(source_type, source_id)` source-lookup index `0058` also created is a prefix of that unique key and is dropped by migration `0080`; source lookups resolve on the unique key.
