...
| Flag | Surface | Broad-rollout state |
|---|---|---|
| `package_registry` | Signed registry, package catalogue/install/update foundation | Deploy-dark; schema landed inert (`0049`), signing-key custody recorded (ADR 0012 A4); implementation pending — next staged increment per `PHASE_5_STATUS.md` |
| `package_themes` | Declarative theme packages, preview, safe mode | Deploy-dark; implementation pending (staged with the registry increment) |
